...
{
    name='downspeedf',				arg='wlan0',                     max_value=54000,
    x=70,							y=423,
    graph_radius=27,
    graph_thickness=5,
    graph_start_angle=180,
...
{
    name='upspeedf',				arg='wlan0',                     max_value=54000,
    x=75,							y=423,
    graph_radius=27,
    graph_thickness=5,
...
{
    name='downspeedf',				arg='eth0',                     max_value=100000,
    x=70,							y=423,
    graph_radius=17,
    graph_thickness=5,
    graph_start_angle=180,
...
{
    name='upspeedf',				arg='eth0',                     max_value=100000,
    x=75,							y=423,
    graph_radius=17,
    graph_thickness=5,
    graph_start_angle=-270,
...
function go_gauge_rings(display)
    ...
    for i in pairs(gauge) do
		if gauge[i]['name'] == 'fs_used_perc' then
			-- only want one
			if gauge[i]['arg'] == '/' then
			-- draw hdd
				-- get mount points
				...
				local local_mountpoints={
					'/',
					'/home',
					'/usr',
					'/var',
					'/srv',
					-- make more lines if you need
				}
				local external_mountpoints = scandir('/media/*','d')
				for mountpi in pairs(local_mountpoints) do
					tmpstr = conky_parse('${fs_type '..local_mountpoints[mountpi]..'}')
					if tmpstr ~= 'unknown' and (tmpstr ~= 'rootfs'or local_mountpoints[mountpi] == '/') then
						--str=str..local_mountpoints[mountpi]
						found.int.mounts[found.int.length] = {
							name = local_mountpoints[mountpi],
							--["type"] = 'int'
						}
						found.int.length = found.int.length +1
					end
				end
				for mountpi in pairs(external_mountpoints) do
					tmpstr = conky_parse('${fs_type '..external_mountpoints[mountpi]..'}')
					if tmpstr ~= 'unknown' and tmpstr ~= 'rootfs' then
						found.ext.mounts[found.ext.length] = {
							name = external_mountpoints[mountpi],
							--["type"] = 'ext'
						}
						found.ext.length = found.ext.length +1
					end
				end
				--parsing the gathered internal partitions
				for datai = 0, found.int.length-1 do
					--str = str..found.int.mounts[datai].name
					angle = (found.ext.length > 0) and 1.8 or 2.7
					graph = {
						name				= 'fs_used_perc',
						arg					= found.int.mounts[datai].name,
						max_value			= 100,
						x					= 85,
						y					= 350,
						graph_radius		= (36 - 6*drawer.length),
						graph_thickness		= 5,
						graph_start_angle	= 180,
						...
						caption				= found.int.mounts[datai].name,
						caption_weight		= 1,
						caption_size		= 5.0,
				...
				for datai = 0, found.ext.length-1 do
					graph = {
						name				= 'fs_used_perc',
						arg					= found.ext.mounts[datai].name,
						max_value			= 100,
						x					= 92,
						y					= (338 - 6 * edrawer.length),
						value_ltr			= false,
						graph_start_angle	= 0,
						graph_width			= 70,
						graph_height		= 0,
						...
						caption				= string.gsub(found.ext.mounts[datai].name,"(/media/)(%w+)","%2",1),
						caption_weight		= 0,
						caption_size		= 7,
						caption_fg_colour	= 0xFFFFFF,
						caption_fg_alpha	= 0.5,
...
-------------------------------------------------------------------------------
--                                                                      scandir
function scandir (directory, options)
	local i, t, popen = 0, {}, io.popen
	local tmp = ''
	for entry in popen('ls -a'..options..' '..directory):lines() do
		i = i + 1
		t[i] = entry
	end
	return t
